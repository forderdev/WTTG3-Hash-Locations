...
const allCrimeOpt = ["Silahlı Saldırı/Ateş Açılması",
                    "Ölümcül Silahla Saldırı",
                    "Ağırlaştırılmış Saldırı",
                    "Gasp",
                    "Araçla Vurup Kaçma",
                    "Ağırlaştırılmış Cinsel Saldırı",
                    "Cinayet",
                    "Adam Kaçırma",
                    "Aile İçi Kavga/Olay",
                    "Polisten Kaçma",
                    ...
                    "Hırsızlık Amacıyla Haneye Girme",
                    "Nitelikli Hırsızlık",
                    "Motorlu Araç Hırsızlığı",
                    "Mala Zarar Verme/Vandalizm",
                    "Kundaklama",
                    "Ağır Suç Kapsamında Uyuşturucu Bulundurma",
                    "Fuhuş"
                  ]
...
for (var i = 0; i < btns.length; i++) {
  btns[i].addEventListener("click", function() {
    if (this.className != "btn active" && totalMult < 5) {
        ...
        const textNode = document.createTextNode(betPick + " | x" + mult + " çarpan");
        node.appendChild(textNode);
        document.getElementById("betsChosen").appendChild(node);
        totalMult = Math.ceil((totalMult + mult) * 100) / 100;
        document.getElementById("betMultiplier").innerHTML = "x" + totalMult + " (toplam bahis çarpanı)";
...
function verifyWager() {
  ...
  if (betToInt === parseInt(betValue, 10) && betToInt >= 25) {
    ...
    document.getElementById("verifyStatus").innerHTML = "Bahis penceresi şu anda kapalı. Yayın başladığında yeniden kontrol et.";
    console.log(document.getElementById("potentialWin").innerHTML);
      ...
      document.getElementById("verifyStatus").innerHTML = "Bahis tutarı asgari miktarın altında.";
  } else {
    console.log("NON INTEGER");
    document.getElementById("verifyStatus").innerHTML = "Alan boş ya da geçersiz karakterler içeriyor; yeniden dene.";
    document.getElementById("betInput").value = "";
  }
}
...
// Takes the current winnings and adds the wheel multiplier to the total mult and displays that result
  ...
  document.getElementById("wheelResult").innerHTML = allCrimeOpt[wheelCrime] + " | + x" + wheelMult + " BONUS ÇARPANI";
  document.getElementById("wheelPotential").innerHTML = "Yeni Toplam Olası Kazanç: " + newPotential + " DOSCoin"
  document.getElementById("bonusPotential").innerHTML = " (" + Math.round((newPotential * 100) - (intWin * 100)) / 100 + " DOSCoin Çark Bonusu dâhil)";
}, 6500);
